...
for acount in range(0,numA):
    ...
    #atimes2[acount] = sum([phix(float(i)/NA)*sum([1.0/(tup(float(z)/NA)*phix(float(z)/NA))  for z in range(0,i+1)])  for i in range(0,N2)] ) 
...
#    atimes[acount] = ph
    ...
    atimes2[acount] = NA*(tup(x1)**-1*(abs(dx12*dx32))**-0.5)*3.142*m.exp(NA*(phi(x3)-phi(x1)))
